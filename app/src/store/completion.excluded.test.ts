...
describe('completion with exclusion', () => {
  ...
  it('drops blocks under an excluded subhead', () => {
    const ex = { sections: new Set<string>(), subsections: new Set(['s#0']), columns: new Set<string>(), rows: new Set<string>() }
    expect(sectionCompletion(sec, {}, ex).total).toBe(1)
  })
  it('overall drops excluded sections', () => {
    const ex = { sections: new Set(['s']), subsections: new Set<string>(), columns: new Set<string>(), rows: new Set<string>() }
